...
def call_judge(prompt, max_tokens=16000):
    from _api_adapter import call_judge as _orig
...
def main():
    if len(sys.argv) == 3:
        ...
        # Full tournament
        chapters = [num for num, _ in discover_chapters(BASE_DIR)]
        if len(chapters) < 2:
            print("Need at least two chapters in chapters/ to compare.")
            sys.exit(1)
