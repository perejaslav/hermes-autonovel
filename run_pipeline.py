...
def run_tool(cmd: str | list[str], timeout: int = 600, check: bool = False) -> subprocess.CompletedProcess:
    ...
    Prefer list commands for shell-free execution. String commands remain
    supported for git and other legacy call sites.
...
def uv_run(script: str, timeout: int = 600) -> subprocess.CompletedProcess:
    ...
    return run_tool([sys.executable, *shlex.split(script)], timeout=timeout)


def require_tool_success(result: subprocess.CompletedProcess, label: str):
    """Abort a pipeline phase when a required tool failed."""
    if result.returncode != 0:
...
def require_nonempty_file(path: Path, label: str, min_size: int = 100):
    """Abort if a required generated file is missing or clearly empty."""
    if not path.exists():
        raise RuntimeError(f"{label} did not create expected file: {path}")
    if path.stat().st_size < min_size:
        raise RuntimeError(f"{label} created an unexpectedly small file: {path}")
...
def run_foundation(state: dict) -> dict:
    ...
    for i in range(iteration + 1, MAX_FOUNDATION_ITERS + 1):
        ...
        result = uv_run("gen_world.py", timeout=300)
        require_tool_success(result, "gen_world.py")
        require_nonempty_file(BASE_DIR / "world.md", "gen_world.py")

        step("Generating characters...")
        result = uv_run("gen_characters.py", timeout=300)
        require_tool_success(result, "gen_characters.py")
        require_nonempty_file(BASE_DIR / "characters.md", "gen_characters.py")

        step("Generating outline (part 1)...")
        result = uv_run("gen_outline.py", timeout=300)
        require_tool_success(result, "gen_outline.py")
        require_nonempty_file(BASE_DIR / "outline.md", "gen_outline.py")

        step("Generating outline (part 2 — foreshadowing)...")
        result = uv_run("gen_outline_part2.py", timeout=300)
        require_tool_success(result, "gen_outline_part2.py")
        require_nonempty_file(BASE_DIR / "outline.md", "gen_outline_part2.py")

        step("Generating canon...")
        result = uv_run("gen_canon.py", timeout=300)
        require_tool_success(result, "gen_canon.py")
        require_nonempty_file(BASE_DIR / "canon.md", "gen_canon.py")

        step("Running voice fingerprint...")
        result = uv_run("voice_fingerprint.py", timeout=300)
        require_tool_success(result, "voice_fingerprint.py")
...
def run_revision(
    ...
    for cycle in range(start_cycle, max_cycles + 1):
        ...
        for idx, item in enumerate(consensus_items):
            ...
            if gen_brief.exists():
                ...
                run_tool([sys.executable, "gen_brief.py", "--panel", str(ch_num)], timeout=300)
    ...
    # =========================================================
    # PHASE 3b: DEEP REVIEW LOOP (prose-level refinement)
    # =========================================================
    review_py = BASE_DIR / "review.py"
    if review_py.exists() and with_deep_review:
        ...
        for rnd in range(1, max_review_rounds + 1):
            ...
            parse_result = run_tool(
                [sys.executable, "review.py", "--parse"], timeout=60)
            ...
            if gen_brief_py.exists():
                ...
                run_tool([sys.executable, "gen_brief.py", "--auto"], timeout=300)
    ...
    elif review_py.exists():
        step("Skipping deep review loop by default; pass --with-deep-review to enable it.")
...
def run_export(state: dict) -> dict:
    ...
    if build_tex.exists():
        ...
        run_tool([sys.executable, "typeset/build_tex.py"], timeout=120)
        ...
        try:
            if phase == "foundation":
                ...
                state = run_revision(
                    ...
                    with_deep_review=args.with_deep_review,
                )
            elif phase == "export":
